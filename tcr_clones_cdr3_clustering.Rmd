...
This script creates and plots networks of TCRs which share sequence similarity as measured by the string kernel, looking for similar triplets. 
...
CDR_exp <- unique(CDR_exp$CDR3_aa.x)

CDR_all <- unique(cd8_np16$CDR3_aa.x)
...
title(main="Expanded CDR3s from NP16 alpha", cex.main=1, font.main=1)
...
# should be character list not dataframe
CDR_exp <- cd8_np16 %>% group_by(CDR3_aa.y) %>% filter(n() >= 2)
CDR_exp <- unique(CDR_exp$CDR3_aa.y)

CDR_all <- unique(cd8_np16$CDR3_aa.y)
...
# Create the plot
plot(km_graph_s, vertex.label=NA,layout=coords.exp,edge.width=1,vertex.color="black",edge.color="grey",vertex.size=4)
title(main="Expanded CDR3s from NP16 beta", cex.main=1, font.main=1)
...
## CD8 ORF3a alpha
```{r, fig.height=6, fig.width=6}
cd8_orf3a <- read.csv("/t1-data/user/lfelce/TCR_analysis/cd8_orf3a-28_tcr_clones.csv")

# this script requires two sets of input; a set of  expanded CDR3 sequences (each CDR3 should only occur once) in the set of TCRs to be analysed, 
# in a variable called CDR_exp; and all the CDR3s (each CDR3 should only occur once) from the same set, in a variable called CDR_all.    

# should be character list not dataframe
CDR_exp <- cd8_orf3a %>% group_by(CDR3_aa.x) %>% filter(n() >= 2)
CDR_exp <- unique(CDR_exp$CDR3_aa.x)

CDR_all <- unique(cd8_orf3a$CDR3_aa.x)
...
# Create the plot
plot(km_graph_s, vertex.label=NA,layout=coords.exp,edge.width=1,vertex.color="black",edge.color="grey",vertex.size=4)
title(main="Expanded CDR3s from ORF3a-28 alpha", cex.main=1, font.main=1)
...
# should be character list not dataframe
CDR_exp <- cd8_orf3a %>% group_by(CDR3_aa.y) %>% filter(n() >= 2)
CDR_exp <- unique(CDR_exp$CDR3_aa.y)

CDR_all <- unique(cd8_orf3a$CDR3_aa.y)
...
# Create the plot
plot(km_graph_s, vertex.label=NA,layout=coords.exp,edge.width=1,vertex.color="black",edge.color="grey",vertex.size=4)
title(main="Expanded CDR3s from ORF3a-28 beta", cex.main=1, font.main=1)
