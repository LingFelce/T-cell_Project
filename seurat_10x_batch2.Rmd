...
```{r}
# Select top PCs for clustering
t2.singlet <- FindNeighbors(t2.singlet, reduction = "pca", dims = 1:7, verbose=FALSE)
t2.singlet <- FindClusters(t2.singlet, resolution = 0.5, verbose = FALSE)
t2.singlet <- RunUMAP(t2.singlet, dims = 1:7, verbose=FALSE)
t2.singlet <- RunTSNE(t2.singlet, reduction = "pca", dims = 1:7, verbose=FALSE)
...
```
#### Subset known demultiplexed cells
```{r}
...
```{r}
# Select top PCs for clustering
t2.known <- FindNeighbors(t2.known, reduction = "pca", dims = 1:5, verbose=FALSE)
t2.known <- FindClusters(t2.known, resolution = 0.5, verbose = FALSE)
t2.known <- RunUMAP(t2.known, dims = 1:5, verbose=FALSE)
t2.known <- RunTSNE(t2.known, reduction = "pca", dims = 1:5, verbose=FALSE)
