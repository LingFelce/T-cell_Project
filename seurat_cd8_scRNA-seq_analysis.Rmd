---
title: "Single-cell RNA-Seq analysis of CD8+ SARS-CoV-2 specific T cells"
output:
  word_document: default
  html_document: default
  pdf_document: default
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
library(Seurat)
library(plyr)
library(dplyr)
library(patchwork)
library(cowplot)
library(ggplot2)
library(data.table)
library(tidyverse)
library(harmony)
library(gridExtra)
```

# All CD8 samples
Analysis of Dong samples: CD8 NP16 & ORF3a-28
Follow guided clustering tutorial https://satijalab.org/seurat/v3.2/pbmc3k_tutorial.html and this one https://scrnaseq-course.cog.sanger.ac.uk/website/seurat-chapter.html


## Load counts matrix and create Seurat object
```{r loading}
# setwd('/t1-data/user/lfelce/scRNA-Seq/SmartSeq2_T-cells/')
# 
# # load data (from featureCounts)
# tcell_data <-  fread('201203_counts.txt', stringsAsFactors = F, header=T)
# 
# # remove columns with chromosome, start, end, strand and length info
# tcell_data <- tcell_data[,-c(2:6)]
# 
# # make Geneid into row names
# tcell_data <-tibble::column_to_rownames(tcell_data, "Geneid")
# 
# # tidy up sample names
# names(tcell_data) <- gsub(x = names(tcell_data), pattern = "./", replacement = "")
# names(tcell_data) <- gsub(x = names(tcell_data), pattern = ".bam", replacement = "")
# 
# # select CD8 cells only but leave out 1131-TP-1 - should have 816
# sample_list <- as.data.frame(colnames(tcell_data))
# tcell_data2 <- tcell_data[,-c(481:486,774:779,1404:1409)]
# sample_list2 <- as.data.frame(colnames(tcell_data2))
# cd8_data <- tcell_data2[,c(1:192, 481:576, 672:1055, 1344:1487)]
# 
# # load new 1131-TP-1 data and process
# data_1131_new <- fread('201221_counts.txt', stringsAsFactors = F, header=T)
# data_1131_new <- data_1131_new[,-c(2:6)]
# data_1131_new <-tibble::column_to_rownames(data_1131_new, "Geneid")
# names(data_1131_new) <- gsub(x = names(data_1131_new), pattern = "./", replacement = "")
# names(data_1131_new) <- gsub(x = names(data_1131_new), pattern = ".bam", replacement = "")
# 
# # leave out bulk samples
# list_1131 <- as.data.frame(colnames(data_1131_new))
# data_1131_new <- data_1131_new[,-c(25:96)]
# 
# # merge with cd8_data = 816 + 92 = 908
# cd8_data <- cbind(cd8_data, data_1131_new)
# 
# # Initialise Seurat object with raw (non-normalised) data
# # min.cells = include features detected in at least this many cells
# # min.features includes cells where at least this many features detected
# cd8 <- CreateSeuratObject(counts = cd8_data, min.cells = 3, min.features = 200, project = "CD8 T-cell_data", assay = "RNA")
```

## Standard pre-processing workflow

### QC and selecting cells for further analysis
Minimum gene cut off already set with CreateSeuratObject, but can filter out cells based on technical or biological parameters.
Visualise gene and molecule counts, plot relationship and exclude cells with clear outlier number of genes detected as potential multiplets (filter user-defined outlier cells). 

Also filter cells based on % mitochondrial genes present:

```{r qc, fig.height=5, fig.width=16}
# # non-UMI data - nUMI represents sum of non-normalised values within cells
# # calculate % mitochondrial genes and store in separate object
# # use object@raw.data represents non-transformed and non-log-normalised counts
# # % of UMI mapping to MT-genes is a common scRNA-seq QC metric
# mito.genes <- grep(pattern = "^MT-", x = rownames(cd8@assays[["RNA"]]), value = TRUE)
# percent.mito <- Matrix::colSums(cd8@assays[["RNA"]][mito.genes, ])/Matrix::colSums(cd8@assays[["RNA"]])
# cd8 <- AddMetaData(object = cd8, metadata = percent.mito, col.name = "percent.mito")
# VlnPlot(object = cd8, features = c("nFeature_RNA", "nCount_RNA", "percent.mito"), ncol = 3)
# # all cells have same value % mitochondrial cells?
```

```{r, fig.height=4, fig.width=6}
# can use GenePlot to visualise gene-gene relationships and anything else calculated by object
# FeatureScatter(object=cd8, feature1="nCount_RNA", feature2="nFeature_RNA")
```


```{r filtering}
# Filter out cells that have unique gene counts (nFeature_RNA) over 4000 or less than 200 
# Note that > and < are used to define a'gate'.  
# -Inf and Inf should be used if don't want a lower or upper threshold.
# cd8 <- subset(x = cd8, subset = nFeature_RNA > 200 & nFeature_RNA < 4000 & percent.mito >  -Inf & percent.mito < 0.05 )
```

### Normalising the data
After removing unwanted cells from dataset, need to normalise data. Use global-scaling normalisation method "LogNormalize" - normalises gene expression measurements for each cell by total expression, multiplies by scale factor (10,000 default) and log-tranforms result.
```{r normalisation}
# cd8 <- NormalizeData(object = cd8, normalization.method = "LogNormalize", scale.factor = 10000)
```

### Detection of variables genes across single cells
Seurat calculates highly variable genes and focuses on these for downstream analysis. FindVariableGenes calculates average expression and dispersion for each gene, places genes into bins and calculates z-score for dispersion within each bin - helps to control relationship between variability and average expression. 

```{r variability, fig.height=8, fig.width = 16}
# identify 2000 variable genes - normal parameter settings for UMI data normalised to total of 1e4 molecules
# cd8 <- FindVariableFeatures(object = cd8, mean.function = ExpMean, dispersion.function = LogVMR, x.low.cutoff = 0.0125, x.high.cutoff = 3, y.cutoff = 0.5, nfeatures = 2000)
# # Identify the 10 most highly variable genes
# top10 <- head(VariableFeatures(cd8), 10)
# # plot variable features with and without labels
# plot1 <- VariableFeaturePlot(cd8)
# plot2 <- LabelPoints(plot = plot1, points = top10, repel = TRUE)
# plot1 + plot2
```
### Scaling data and removing unwanted sources of variation
Dataset may include uninteresting sources of variation - technical noise, batch effects, biological sources of variation (cell cycle) - regressing these signals can improve downstream dimensionality reduction and clustering. To judge effects of these signals, Seurat constructs linear models to predict gene expression based on user-defined variables. Scaled z-scored residuals of these models are stored in scale.data slot, used for dimensionality reduction and clustering.

```{r scaling}
# cd8 <- ScaleData(object = cd8, vars.to.regress = c("nCounts_RNA", "percent.mito"))
```

### Linear dimensional reduction (PCA)
Perform PCA on scaled data. Not sure if highly variable genes or pc.genes are input?

```{r pca, fig.height=4, fig.width=6}
# cd8 <- RunPCA(object = cd8,  npcs = 30, verbose = FALSE)
# #standard PCA plot
# DimPlot(object = cd8, reduction = "pca")
```

Using a heatmap can be useful to look at primary sources of heterogeneity in a dataset, and can use to decide which PCs to include for further downstream analysis. 

```{r pca heatmap, fig.height=4, fig.width=6}
# DimHeatmap(object = cd8, reduction = "pca", cells = 200, balanced = TRUE)
```

### Determine statistically significant principal components
Overcome extensive technical noise in any single gene for scRNA-seq data, Seurat clusters cells based on PCA scores. Determining how many PCs to include downstream is an important step.

Here it looks like PC1-8 are significant (looking at p-values)
```{r jackstraw, fig.height=4, fig.width=6}
# cd8 <- JackStraw(cd8, num.replicate = 100)
# # visualise distribution of p-values for each PC with uniform distribution (dashed line) Significant PCs show strong enrichment of genes with low p-values (solid curve above dashed line)
# cd8 <- ScoreJackStraw(cd8, dims = 1:20)
# JackStrawPlot(cd8, dims = 1:15)
```


## Cell clustering
Construct a k-nearest neighbours graph based on Euclidean distance in PCA space, refine edge weights between any 2 cells based on shared overlap in local neighbourhoods. Use Louvain algorithm to cluster cells - modularity optimisation technique to iteratively group cells together.

Resolution parameter - sets granularity of downstream clustering, larger values leads to more clusters. Set between 0.6-1.2 for single cell datasets around 3000 cells. 

```{r clustering}
# calculate KNN and construct SNN graph (find neighbours), then find clusters
# cd8 <- FindNeighbors(cd8, dims = 1:8)
# cd8 <- FindClusters(cd8, resolution = 0.4)
```
### Non-linear dimensional reduction (UMAP)
Place similar cells in low-dimensional space. Use same PCs as input to clustering analysis
```{r umap, fig.height=5, fig.width=16}
# cd8 <- RunUMAP(cd8, dims = 1:8)
# # plot UMAP and labels by cluster
# Idents(cd8) <- "seurat_clusters"
# UMAP_cluster <- DimPlot(cd8, reduction = "umap", group.by = "seurat_clusters")
# # plot UMAP and label by patient
# UMAP_patient <- DimPlot(cd8, reduction = "umap", group.by = "orig.ident")
# UMAP_cluster + UMAP_patient
```

## Finding differentially expressed genes (cluster biomarkers)
Define clusters via differential expression. Identifies positive and negative markers of single cluster compared to all other cells.

```{r number of cells}
# How many cells for each patient?
# Idents(cd8) <- "orig.ident"
# table(Idents(cd8))
# # How many cells in each cluster?
# Idents(cd8) <- "seurat_clusters"
# table(Idents(cd8))
```


```{r markers}
# find markers for every cluster compared to remaining cells, report only positive ones
# Wilcoxon Rank Sum test default
# cd8.markers <- FindAllMarkers(object = cd8, only.pos = TRUE, min.pct = 0.25, thresh.use = 0.25)
# write.csv (cd8.markers, "cd8-tcell_markers.csv")
# divide file into separate clusters
# cluster_0 <- cd8.markers[cd8.markers$cluster == "0", ]
# cluster_1 <- cd8.markers[cd8.markers$cluster == "1", ]
# cluster_2 <- cd8.markers[cd8.markers$cluster == "2", ]
# cluster_3 <- cd8.markers[cd8.markers$cluster == "3", ]
# write.csv (cluster_0, "cluster_0.csv")
# write.csv (cluster_1, "cluster_1.csv")
# write.csv (cluster_2, "cluster_2.csv")
# write.csv (cluster_3, "cluster_3.csv")
# show top 2 differentially expressed genes per cluster
# cd8.markers %>% group_by(cluster) %>% top_n(n = 2, wt = avg_log2FC)
```


```{r heatmap}
# Expression heatmap for top 20 markers for each cluster
# top10 <- cd8.markers %>% group_by(cluster) %>% top_n(10, avg_log2FC)
# setting slim.col.label to TRUE will print just the cluster IDS instead of every cell name
# DoHeatmap(object = cd8, features = top10$gene, label = TRUE)
```

# Annotating UMAPs with meaningful information
## Known information

```{r}
# list <- as.data.frame(cd8$orig.ident)
# 
# list$epitope <- ifelse(grepl("005|1131|1153|1201", list$`cd8$orig.ident`), "NP16",
#                        ifelse(grepl("1105|1134|1525", list$`cd8$orig.ident`), "ORF3a-28", ""))
# 
# list$clin.type <- ifelse(grepl("005|1201", list$`cd8$orig.ident`), "mild",
#                          ifelse(grepl("1131|1153|1105|1134|1525", list$`cd8$orig.ident`), "severe",""))
# 
# cd8@meta.data$epitope <- as.factor(list$epitope)
# cd8@meta.data$clin.type <- as.factor(list$clin.type)

```

```{r, fig.height=4, fig.width=10}
# UMAP_clin <- DimPlot(cd8, reduction = "umap", group.by = "clin.type", label = TRUE, label.size = 3 ,repel = TRUE)
# 
# UMAP_epitope <- DimPlot(cd8, reduction = "umap", group.by = "epitope", label = TRUE, label.size = 3 ,repel = TRUE)
# 
# # use split.by="seurat_clusters" to also show UMAP by epitope and cluster number
# 
# UMAP_clin + UMAP_epitope
# 
# saveRDS(cd8, "cd8_seurat.rds")
```


## Gene expression by epitope

```{r, fig.height=32, fig.width=16}
# VlnPlot(object = cd8, features = c("CD27", "IL7R", "S100A4", "PTPRC", 
#                                    "CCR7", "SELL", "CD44", "CD69", 
#                                    "HLA-DRB1", "CCL4", 
#                                    "CCL5", "CXCR3", "CXCR4",
#                                    "IL16", "IFNG", 
#                                    "PRF1", "GNLY", "NKG7", 
#                                    "GZMA", "GZMB", "GZMH", "GZMK",
#                                    "FASLG", "ISG15",
#                                    "PDCD1", "CTLA4", "HAVCR2", "CD244",
#                                    "KLRG1", "TIGIT", "BTLA", "CD160"), 
#         ncol=4, group.by = "epitope")
```

# Integrate CD8 patients using Harmony (recommended by Adam)
Harmony integration carried out on BMRC cluster R studio with locally installed Harmony package and Seurat.
Loaded cd8_seurat.rds from 28th December 2020 for previously created and analysed cd8 Seurat object.

http://htmlpreview.github.io/?https://github.com/immunogenomics/harmony/blob/master/docs/SeuratV3.html
IMPORTANT DIFFERENCE: In the Seurat integration tutorial, you need to define a Seurat object for each dataset. With Harmony integration, create only one Seurat object with all cells.

The simplest way to run Harmony is to pass the Seurat object and specify which variable(s) to integrate out. RunHarmony returns a Seurat object, updated with the corrected Harmony coordinates. Let's set plot_convergence to TRUE, so we can make sure that the Harmony objective function gets better with each round.

```{r harmony1}
# cd8 <- readRDS("cd8_seurat.rds")
# # options(repr.plot.height = 2.5, repr.plot.width = 6)
# cd8 <- cd8 %>%
#     RunHarmony("orig.ident", plot_convergence = TRUE)
# # Harmony converged after 8 iterations
# # access Harmony embeddings - show top 5 rows and top 5 columns
# harmony_embeddings <- Embeddings(cd8, 'harmony')
```

```{r harmony2, fig.height=5, fig.width=12}
# p1 <- DimPlot(object = cd8, reduction = "harmony", group.by = "orig.ident")
# p2 <- VlnPlot(object = cd8, features = "harmony_1", group.by = "orig.ident", pt.size = .1)
# p1 + p2
```
```{r harmony3, fig.height=5, fig.width=12}
# p3 <- DimPlot(object = cd8, reduction = "pca", group.by="orig.ident")
# p3 + p1
```

Many downstream analyses are performed on low dimensional embeddings, not gene expression. To use the corrected Harmony embeddings rather than PCs, set reduction = 'harmony'. For example, let's perform the UMAP and Nearest Neighbor analyses using the Harmony embeddings.
```{r harmony_umap, fig.height=14, fig.width=14}
# cd8 <- readRDS("cd8_harmony.rds")
# 
# cd8 <- cd8 %>%
#     RunUMAP(reduction = "harmony", dims = 1:25, verbose=FALSE) %>%
#     FindNeighbors(reduction = "harmony", dims = 1:25, verbose=FALSE) %>%
#     FindClusters(resolution = 0.5, verbose=FALSE) %>%
#     identity()
# 
# # list <- as.data.frame(cd8$orig.ident)
# # list$timepoint <- ifelse(grepl("TP-1", list$`cd8$orig.ident`), "acute",
# #                          ifelse(grepl("005|1131-TP-2|1153|1201|1105|1134|1525-TP-2", 
# #                                       list$`cd8$orig.ident`), "convalescent", ""))
# # 
# # cd8@meta.data$"timepoint" <- as.factor(list$timepoint)
# 
# p1 <- DimPlot(cd8, reduction = "umap", group.by = "orig.ident")
# p2 <- DimPlot(cd8, reduction = "umap", group.by = "epitope")
# p3 <- DimPlot(cd8, reduction = "umap", group.by = "seurat_clusters")
# p4 <- DimPlot(cd8, reduction = "umap", group.by = "timepoint")
# p5 <- DimPlot(cd8, reduction = "umap", group.by = "clin.type")
# 
# grid.arrange(p1, p2, p3, p4, p5, nrow=3)
 
# saveRDS(cd8, "cd8_harmony.rds")

```

```{r harmony_markers}
# cd8.markers <- FindAllMarkers(object = cd8, only.pos = TRUE, min.pct = 0.25, thresh.use = 0.25)
# cluster_0 <- cd8.markers[cd8.markers$cluster == "0", ]
# cluster_1 <- cd8.markers[cd8.markers$cluster == "1", ]
# cluster_2 <- cd8.markers[cd8.markers$cluster == "2", ]
# cluster_3 <- cd8.markers[cd8.markers$cluster == "3", ]
# 
# 
# setwd('/t1-data/user/lfelce/scRNA-Seq/SmartSeq2_T-cells/')
# tmarkers <- read.csv("Markers_for_Ling.csv", header=F)
# 
# genes_0 <- tmarkers[is.element(tmarkers$V1, cluster_0$gene),]
# genes_1 <- tmarkers[is.element(tmarkers$V1, cluster_1$gene),]
# genes_2 <- tmarkers[is.element(tmarkers$V1, cluster_2$gene),]
# genes_3 <- tmarkers[is.element(tmarkers$V1, cluster_3$gene),]


```


```{r}
# # Expression heatmap for top 20 markers for each cluster
# top10 <- cd8.markers %>% group_by(cluster) %>% top_n(10, avg_log2FC)
# # setting slim.col.label to TRUE will print just the cluster IDS instead of every cell name
# DoHeatmap(object = cd8, features = top10$gene, label = TRUE)

```



# Acute patients
Analyse acute patients separately - 1131-TP-1 (NP16) and 1525-TP-1 (ORF3a-28)
## Processing data
```{r, fig.width=10, fig.height=4}
# # select only columns containing TP-1
# acute_data <- select(cd8_data,contains("TP-1"))
# acute <- CreateSeuratObject(counts = acute_data, min.cells = 3, min.features = 200, project = "CD8 acute T-cell data", assay = "RNA")
# 
# # non-UMI data - nUMI represents sum of non-normalised values within cells
# # calculate % mitochondrial genes and store in separate object
# # use object@raw.data represents non-transformed and non-log-normalised counts
# # % of UMI mapping to MT-genes is a common scRNA-seq QC metric
# mito.genes <- grep(pattern = "^MT-", x = rownames(acute@assays[["RNA"]]), value = TRUE)
# 
# percent.mito <- Matrix::colSums(acute@assays[["RNA"]][mito.genes, ])/Matrix::colSums(acute@assays[["RNA"]])
# 
# acute <- AddMetaData(object = acute, metadata = percent.mito, col.name = "percent.mito")
# 
# VlnPlot(object = acute, features = c("nFeature_RNA", "nCount_RNA", "percent.mito"), ncol = 3)
# # all cells have same value % mitochondrial cells?
```

```{r, fig.height=4, fig.width=6}
# can use GenePlot to visualise gene-gene relationships and anything else calculated by object
# FeatureScatter(object=acute, feature1="nCount_RNA", feature2="nFeature_RNA")
```


```{r, fig.height=8, fig.width = 16}
# # Filter out cells that have unique gene counts (nFeature_RNA) over 4000 or less than 200
# # Note that > and < are used to define a'gate'.
# # -Inf and Inf should be used if don't want a lower or upper threshold.
# acute <- subset(x = acute, subset = nFeature_RNA > 200 & nFeature_RNA < 4000 & percent.mito >  -Inf & percent.mito < 0.05 )
# 
# # After removing unwanted cells from dataset, need to normalise data. Use global-scaling normalisation method "LogNormalize" - normalises gene expression measurements for each cell by total expression, multiplies by scale factor (10,000 default) and log-tranforms result.
# 
# acute <- NormalizeData(object = acute, normalization.method = "LogNormalize", scale.factor = 10000)
# 
# # identify 2000 variable genes - normal parameter settings for UMI data normalised to total of 1e4 molecules
# acute <- FindVariableFeatures(object = acute, mean.function = ExpMean, dispersion.function = LogVMR, x.low.cutoff = 0.0125, x.high.cutoff = 3, y.cutoff = 0.5, nfeatures = 2000)
# 
# # Identify the 10 most highly variable genes
# top10 <- head(VariableFeatures(acute), 10)
# 
# # plot variable features with and without labels
# plot1 <- VariableFeaturePlot(acute)
# plot2 <- LabelPoints(plot = plot1, points = top10, repel = TRUE)
# plot1 + plot2
```



```{r, fig.height=4, fig.width=6}
# acute <- ScaleData(object = acute, vars.to.regress = c("nCounts_RNA", "percent.mito"))
# 
# acute <- RunPCA(object = acute,  npcs = 30, verbose = FALSE)
# 
# # standard PCA plot
# p3 <- DimPlot(object = acute, reduction = "pca")
# p3
```
## Harmony integration 
```{r}
# acute <- acute %>%
#     RunHarmony("orig.ident", plot_convergence = TRUE)
# # Harmony converged after 3 iterations
# harmony_embeddings <- Embeddings(acute, 'harmony')

```

```{r, fig.height=5, fig.width=20}
# p1 <- DimPlot(object = acute, reduction = "harmony", group.by = "orig.ident")
# p2 <- VlnPlot(object = acute, features = "harmony_1", group.by = "orig.ident", pt.size = .1)
# p3 + p1 + p2
```
```{r, fig.height=4, fig.width=10}
# acute <- acute %>%
#     RunUMAP(reduction = "harmony", dims = 1:20) %>%
#     FindNeighbors(reduction = "harmony", dims = 1:20) %>%
#     FindClusters(resolution = 0.5) %>%
#     identity()
# umap_patient <- DimPlot(acute, reduction = "umap", group.by = "orig.ident")
# umap_cluster <- DimPlot(acute, reduction = "umap", group.by = "seurat_clusters")
# umap_patient + umap_cluster
```

```{r}
# acute.markers <- FindAllMarkers(object = acute, only.pos = TRUE, min.pct = 0.25, thresh.use = 0.25)
# 
# cluster_0 <- acute.markers[acute.markers$cluster == "0", ]
# cluster_1 <- acute.markers[acute.markers$cluster == "1", ]
# 
# tmarkers <- read.csv("Markers_for_Ling.csv", header=F)
# 
# genes_0 <- tmarkers[is.element(tmarkers$V1, cluster_0$gene),]
# genes_1 <- tmarkers[is.element(tmarkers$V1, cluster_1$gene),]


```



# Convalescent patients
Analyse convalescent patients separately - all patients except 1131-TP-1 and 1525-TP-1
## Processing data
```{r, fig.height=5, fig.width=15}
# remove any columns containing TP-1
# conv_data <- cd8_data %>% select(!(contains("TP-1")))
# 
# conv <- CreateSeuratObject(counts = conv_data, min.cells = 3, min.features = 200, project = "CD8 conv T-cell data", assay = "RNA")
# 
# # non-UMI data - nUMI represents sum of non-normalised values within cells
# # calculate % mitochondrial genes and store in separate object
# # use object@raw.data represents non-transformed and non-log-normalised counts
# # % of UMI mapping to MT-genes is a common scRNA-seq QC metric
# 
# mito.genes <- grep(pattern = "^MT-", x = rownames(conv@assays[["RNA"]]), value = TRUE)
# 
# percent.mito <- Matrix::colSums(conv@assays[["RNA"]][mito.genes, ])/Matrix::colSums(conv@assays[["RNA"]])
# 
# conv <- AddMetaData(object = conv, metadata = percent.mito, col.name = "percent.mito")
# 
# VlnPlot(object = conv, features = c("nFeature_RNA", "nCount_RNA", "percent.mito"), ncol = 3)
# # all cells have same value % mitochondrial cells?
```

```{r, fig.height=4, fig.width=6}
# can use GenePlot to visualise gene-gene relationships and anything else calculated by object
# FeatureScatter(object=conv, feature1="nCount_RNA", feature2="nFeature_RNA")
```


```{r, fig.height=8, fig.width = 16}
# Filter out cells that have unique gene counts (nFeature_RNA) over 4000 or less than 200
# Note that > and < are used to define a'gate'.
# -Inf and Inf should be used if don't want a lower or upper threshold.
# conv <- subset(x = conv, subset = nFeature_RNA > 200 & nFeature_RNA < 4000 & percent.mito >  -Inf & percent.mito < 0.05 )
# 
# # After removing unwanted cells from dataset, need to normalise data. Use global-scaling normalisation method "LogNormalize" - normalises gene expression measurements for each cell by total expression, multiplies by scale factor (10,000 default) and log-tranforms result.
# conv <- NormalizeData(object = conv, normalization.method = "LogNormalize", scale.factor = 10000)
# 
# # identify 2000 variable genes - normal parameter settings for UMI data normalised to total of 1e4 molecules
# conv <- FindVariableFeatures(object = conv, mean.function = ExpMean, dispersion.function = LogVMR, x.low.cutoff = 0.0125, x.high.cutoff = 3, y.cutoff = 0.5, nfeatures = 2000)
# 
# # Identify the 10 most highly variable genes
# top10 <- head(VariableFeatures(conv), 10)
# 
# # plot variable features with and without labels
# plot1 <- VariableFeaturePlot(conv)
# plot2 <- LabelPoints(plot = plot1, points = top10, repel = TRUE)
# plot1 + plot2
```



```{r, fig.height=4, fig.width=6}
# conv <- ScaleData(object = conv, vars.to.regress = c("nCounts_RNA", "percent.mito"))
# conv <- RunPCA(object = conv,  npcs = 30, verbose = FALSE)
# 
# # standard PCA plot
# p3 <- DimPlot(object = conv, reduction = "pca")
# p3
```
## Harmony integration 
```{r}
# conv <- conv %>%
#     RunHarmony("orig.ident", plot_convergence = TRUE)
# # Harmony converged after 7 iterations
# harmony_embeddings <- Embeddings(conv, 'harmony')
```

```{r, fig.height=5, fig.width=20}
# p1 <- DimPlot(object = conv, reduction = "harmony", group.by = "orig.ident")
# p2 <- VlnPlot(object = conv, features = "harmony_1", group.by = "orig.ident", pt.size = .1)
# p3 + p1 + p2
```

```{r, fig.height=5, fig.width=12}
# conv <- conv %>%
#     RunUMAP(reduction = "harmony", dims = 1:25) %>%
#     FindNeighbors(reduction = "harmony", dims = 1:25) %>%
#     FindClusters(resolution = 0.5) %>%
#     identity()
# 
# 
# epitope <- rep(c("NP16", "ORF3a-28", "NP16", "ORF3a-28", "NP16", "NP16", "ORF3a-28"),
#                  times = c("133", "94", "74", "95", "66", "75", "95"))
# conv@meta.data$"epitope" <- as.factor(epitope)
# 
# umap_patient <- DimPlot(conv, reduction = "umap", group.by = "orig.ident")
# umap_cluster <- DimPlot(conv, reduction = "umap", group.by = "seurat_clusters")
# umap_eptiope <- DimPlot(conv, reduction = "umap", group.by = "epitope")
# umap_patient + umap_cluster
```

```{r}
# conv.markers <- FindAllMarkers(object = conv, only.pos = TRUE, min.pct = 0.25, thresh.use = 0.25)
# cluster_0 <- conv.markers[conv.markers$cluster == "0", ]
# cluster_1 <- conv.markers[conv.markers$cluster == "1", ]
# cluster_2 <- conv.markers[conv.markers$cluster == "2", ]
# cluster_3 <- conv.markers[conv.markers$cluster == "3", ]
# 
# tmarkers <- read.csv("Markers_for_Ling.csv", header=F)
# 
# genes_0 <- tmarkers[is.element(tmarkers$V1, cluster_0$gene),]
# genes_1 <- tmarkers[is.element(tmarkers$V1, cluster_1$gene),]
# genes_2 <- tmarkers[is.element(tmarkers$V1, cluster_2$gene),]
# genes_3 <- tmarkers[is.element(tmarkers$V1, cluster_3$gene),]
# 
# 
# cluster1.markers <- FindMarkers(conv, ident.1 = 1, ident.2 = 0, min.pct = 0.25)
# cluster1.markers <- rownames_to_column(cluster1.markers, var = "gene")
# genes_1 <- tmarkers[is.element(tmarkers$V1, cluster_1$gene),]
```

# Chemokine/cytokine and integrin expression on UMAPs
Load each previously saved .rds file - no need to repeat harmony integration, clustering etc.

## All 9 CD8+ patients
```{r, fig.height=10, fig.width=16}
setwd('/t1-data/user/lfelce/scRNA-Seq/SmartSeq2_T-cells/')
cd8 <- readRDS("cd8_harmony.rds")

FeaturePlot(object = cd8, features = c("SELPLG", "ITGA4", "ITGAL", "ITGB2", "CCL4", "CCL5", "CXCR3", "CXCR4", "CCR2" ), cols = c("grey", "blue"), reduction = "umap")
```

```{r, fig.height=10, fig.width=20}
FeaturePlot(object = cd8, features = c("HAVCR2", "IFNG", "ITGAE", "PDCD1", "CXCL13", "LAYN", "LAG3", "TIGIT", "CTLA4", "ENTPD1"), cols = c("grey", "blue"), reduction = "umap")
```

```{r, fig.height=14, fig.width=20}
FeaturePlot(object = cd8, features = c("PRF1", "GZMA", "GZMH", "GZMK", "GZMB", "GNLY", "FGFBP2", "KLRG1", "GZMM", "LYAR", "FCRL6", "NKG7", "KLRD1", "CD74"), cols = c("grey", "blue"), reduction = "umap")
```

## Acute patients
```{r, fig.height=10, fig.width=16}
# acute <- readRDS("acute_harmony.rds")
# 
# FeaturePlot(object = acute, features = c("SELPLG","ITGA4", "ITGAL", "ITGB2", "CCL4", "CCL5", "CXCR3", "CXCR4"), cols = c("grey", "blue"), reduction = "umap")
```

## Convalescent patients
```{r, fig.height=10, fig.width=16}
# conv <- readRDS("convalescent_harmony.rds")
# 
# FeaturePlot(object = conv, features = c("SELPLG","ITGA4", "ITGAL", "ITGB2", "CCL4", "CCL5", "CXCR3", "CXCR4"), cols = c("grey", "blue"), reduction = "umap")
```
## Convalescent patients UMAP epitope
```{r}
# DimPlot(conv, reduction = "umap", group.by = "epitope")
```
# Gene expression violin plots
## All CD8+ patients

```{r, fig.height=24, fig.width=18}
# VlnPlot(object = cd8, features=c("PTPRC", "CCR7", "SELL", "TCF7", "IL7R", "S100A4", 
#                                   "CD44","CD27", "CD28", "CD69", "CD38","HLA-DRB1",
#                                   "IFNG", "CCL4", "CCL5", "CCR4", "CXCR3", "CXCR4",                                                   "SELPLG", "ITGA4", "ITGAL", "ITGB2", "ITGAE", "ICAM1",
#                                   "GZMA", "GZMB", "GZMH", "GZMK", "PRF1", "GNLY",
#                                   "NKG7", "FASLG", "FAS",  "ISG15", "MX1", "OAS1", 
#                                   "TUBA1B","LAG3","PDCD1", "HAVCR2", "TIGIT", "CTLA4",
#                                  "KLRD1", "FGFBP2", "KLRC1", "KLRC3", "KLRB1", "KLRC2"),
#         ncol=6, group.by = "seurat_clusters")
```

```{r, fig.height=26, fig.width=18}
# VlnPlot(object = cd8, features=c("PTPRC", "CCR7", "SELL", "TCF7", "IL7R", "S100A4", 
#                                   "CD44","CD27", "CD28", "CD69", "CD38","HLA-DRB1",
#                                   "IFNG", "CCL4", "CCL5", "CCR4", "CXCR3", "CXCR4",                                                   "SELPLG", "ITGA4", "ITGAL", "ITGB2", "ITGAE", "ICAM1",
#                                   "GZMA", "GZMB", "GZMH", "GZMK", "PRF1", "GNLY",
#                                   "NKG7", "FASLG", "FAS",  "ISG15", "MX1", "OAS1", 
#                                   "TUBA1B","LAG3","PDCD1", "HAVCR2", "TIGIT", "CTLA4",
#                                  "KLRD1", "FGFBP2", "KLRC1", "KLRC3", "KLRB1", "KLRC2"),
#         ncol=6, group.by = "epitope")
```

```{r, fig.height=24, fig.width=18}
# VlnPlot(object = cd8, features=c("PTPRC", "CCR7", "SELL", "TCF7", "IL7R", "S100A4", 
#                                   "CD44","CD27", "CD28", "CD69", "CD38","HLA-DRB1",
#                                   "IFNG", "CCL4", "CCL5", "CCR4", "CXCR3", "CXCR4",                                                   "SELPLG", "ITGA4", "ITGAL", "ITGB2", "ITGAE", "ICAM1",
#                                   "GZMA", "GZMB", "GZMH", "GZMK", "PRF1", "GNLY",
#                                   "NKG7", "FASLG", "FAS",  "ISG15", "MX1", "OAS1", 
#                                   "TUBA1B","LAG3","PDCD1", "HAVCR2", "TIGIT", "CTLA4",
#                                  "KLRD1", "FGFBP2", "KLRC1", "KLRC3", "KLRB1", "KLRC2"),
#         ncol=6, group.by = "clin.type")
```

```{r, fig.height=26, fig.width=18}
# VlnPlot(object = cd8, features=c("PTPRC", "CCR7", "SELL", "TCF7", "IL7R", "S100A4", 
#                                   "CD44","CD27", "CD28", "CD69", "CD38","HLA-DRB1",
#                                   "IFNG", "CCL4", "CCL5", "CCR4", "CXCR3", "CXCR4",                                                   "SELPLG", "ITGA4", "ITGAL", "ITGB2", "ITGAE", "ICAM1",
#                                   "GZMA", "GZMB", "GZMH", "GZMK", "PRF1", "GNLY",
#                                   "NKG7", "FASLG", "FAS",  "ISG15", "MX1", "OAS1", 
#                                   "TUBA1B","LAG3","PDCD1", "HAVCR2", "TIGIT", "CTLA4",
#                                  "KLRD1", "FGFBP2", "KLRC1", "KLRC3", "KLRB1", "KLRC2"),
#         ncol=6, group.by = "timepoint")
```

## Acute patients
### Gene expression by patient/epitope

```{r, fig.height=36, fig.width=16}
# VlnPlot(object = acute, features = c("CD27", "IL7R", "S100A4", "PTPRC",
#                                     "CCR7", "SELL", "CD44", "CD69",
#                                     "HLA-DRB1", "CCL4",
#                                     "CCL5", "CXCR3", "CXCR4",
#                                     "SELPLG","ITGA4", "ITGAL", "ITGB2",
#                                     "IL16", "IFNG",
#                                     "PRF1", "GNLY", "NKG7",
#                                     "GZMA", "GZMB", "GZMH", "GZMK",
#                                     "FASLG", "ISG15",
#                                     "PDCD1", "CTLA4", "HAVCR2", "CD244",
#                                     "KLRG1", "TIGIT", "BTLA","CD160"),
#         ncol=4, group.by = "orig.ident")
```
```{r, fig.height=24, fig.width=16}
# VlnPlot(object = acute, features = c("CXCR3", "CXCR4", "CCR4", "CCR5", "CCR6", "CXCR6",
#                                    "PDCD1", "CTLA4", "HAVCR2", "CD244", "KLRG1", "TIGIT", 
#                                    "BTLA", "CD160",
#                                    "CD28", "CD27", "ICOS", "TNFRSF14", "TNFSF14", "CD40LG", 
#                                    "TNFRSF9", "TNFRSF4",
#                                    "TNFRSF25", "TNFRSF18", "TNFRSF8", "HAVCR1", "CD2", "CD226", 
#                                    "LILRB1","LILRB2", "LAIR1"),
#         ncol=4, group.by = "orig.ident")


```

### Gene expression by cluster
```{r, fig.height=32, fig.width=16}
# VlnPlot(object = acute, features = c("CD27", "IL7R", "S100A4", "PTPRC",
#                                     "CCR7", "SELL", "CD44", "CD69",
#                                     "HLA-DRB1", "CCL4",
#                                     "CCL5", "CXCR3", "CXCR4",
#                                     "SELPLG","ITGA4", "ITGAL", "ITGB2",
#                                     "IL16", "IFNG",
#                                     "PRF1", "GNLY", "NKG7",
#                                     "GZMA", "GZMB", "GZMH", "GZMK",
#                                     "FASLG", "ISG15",
#                                     "PDCD1", "CTLA4", "HAVCR2", "CD244",
#                                     "KLRG1", "TIGIT", "BTLA","CD160"),
#         ncol=4, group.by = "seurat_clusters")
```




## Convalescent patients
### Gene expression by cluster
```{r, fig.height=32, fig.width=16}
# VlnPlot(object = conv, features = c("CD27", "IL7R", "S100A4", "PTPRC",
#                                     "CCR7", "SELL", "CD44", "CD69",
#                                     "HLA-DRB1", "CCL4",
#                                     "CCL5", "CXCR3", "CXCR4",
#                                     "SELPLG","ITGA4", "ITGAL", "ITGB2",
#                                     "IL16", "IFNG",
#                                     "PRF1", "GNLY", "NKG7",
#                                     "GZMA", "GZMB", "GZMH", "GZMK",
#                                     "FASLG", "ISG15",
#                                     "PDCD1", "CTLA4", "HAVCR2", "CD244",
#                                     "KLRG1", "TIGIT", "BTLA","CD160"), 
#         ncol=4, group.by = "seurat_clusters")
```
### Gene expression by epitope
```{r, fig.height=18, fig.width=16}
# 
# epitope <- rep(c("NP16", "ORF3a-28", "NP16", "ORF3a-28", "NP16", "NP16", "ORF3a-28"),
#                  times = c("133", "94", "74", "95", "66", "75", "95"))
# conv@meta.data$"epitope" <- as.factor(epitope)

# conv <- readRDS("convalescent_harmony.rds")
# 
# VlnPlot(object = conv, features=c("PTPRC", "CCR7", "SELL", "TCF7", "IL7R", "S100A4", 
#                                   "CD44","CD27", "CD28", "CD69", "CD38","HLA-DRB1",
#                                   "IFNG", "TNF", "CCL4", "CCL5", "CCR4", "CXCR3", "CXCR4",                                            "SELPLG", "ITGA4", "ITGAL", "ITGB2", "ITGAE", 
#                                   "GZMA", "GZMB", "GZMH", "GZMK", "PRF1", "GNLY",
#                                    "NKG7",  "ISG15", "TUBA1B","HAVCR2", "TIGIT", "CTLA4"),
#         ncol=6, group.by = "epitope")

```

```{r, fig.height=28, fig.width=16}
# VlnPlot(object = conv, features = c("CXCR3", "CXCR4", "CCR4", "CCR5", "CCR6", "CXCR6",
#                                    "PDCD1", "CTLA4", "HAVCR2", "CD244", "KLRG1", "TIGIT", 
#                                    "BTLA", "CD160",
#                                    "CD28", "CD27", "ICOS", "TNFRSF14", "TNFSF14", "CD40LG", 
#                                    "TNFRSF9", "TNFRSF4",
#                                    "TNFRSF25", "TNFRSF18", "TNFRSF8", "HAVCR1", "CD2", "CD226", 
#                                    "LILRB1","LILRB2", "LAIR1"),
#         ncol=4, group.by = "epitope")


```

# Adding TCR grouping from clones and single cells
```{r}
# cd8 <- readRDS("cd8_harmony.rds")
# 
# # cd8$CDR3_group <- NULL
# 
# cells <- as.data.frame(cd8$orig.ident)
# cells <- tibble::rownames_to_column(cells, "cell_name")
# colnames(cells) <- c("cell_name", "patient")
# groups <- read.csv("/t1-data/user/lfelce/scRNA-Seq/SmartSeq2_T-cells/cd8_np16_orf3a_sc_groups.csv", header=TRUE)
# 
# # add 0 in front of single digit numbers
# groups$new.group <- str_pad(groups$new.group, 2, side="left", pad="0")
# 
# cell_groups <- merge(cells, groups, by.x="cell_name", by.y="name", all.x=TRUE)
# # cell_groups <- cell_groups[-280,]
# # cell_groups <- cell_groups[!duplicated(cell_groups$cell_name),]
# 
# # re-arrange order so that 1131-TP-1 rows are at bottom
# cell_groups <- cell_groups[c(1:227, 290:740, 228:289),]
# 
# cell_groups$epitope <- ifelse(grepl("005|1131|1153|1201", cell_groups$patient), "NP16",
#                               ifelse(grepl("1105|1134|1525", cell_groups$patient), "ORF3a-28",""))
# 
# cell_groups <- mutate(cell_groups, group=paste(epitope, new.group, sep="_"))
# 
# cell_groups$avidity <- ifelse(grepl("NP16_01|NP16_05|NP16_06|NP16_07|NP16_08|NP16_09|NP16_10|NP16_12|NP16_16|NP16_17|NP16_18|NP16_19", cell_groups$group), "NP16 low avidity",
#                        ifelse(grepl("NP16_02|NP16_03|NP16_04|NP16_13|NP16_14|NP16_15|NP16_20", cell_groups$group), "NP16 high avidity",
#                        ifelse(grepl("ORF3a-28_01|ORF3a-28_05|ORF3a-28_06|ORF3a-28_08", cell_groups$group), "ORF3a-28 low avidity",
#                        ifelse(grepl("ORF3a-28_02|ORF3a-28_03|ORF3a-28_04|ORF3a-28_07", cell_groups$group), "ORF3a-28 high avidity",
#                        ifelse(grepl("NP16_NA|ORF3a-28_NA", cell_groups$group), "NA", "")))))
# 
# cd8@meta.data$CDR3_group <- as.factor(cell_groups$group)
# cd8@meta.data$avidity <- as.factor(cell_groups$avidity)

```

```{r, fig.width=46, fig.height=20}
# cd8_2 <- subset(cd8, subset = CDR3_group !="NP16_NA")
# 
# cd8_2 <- subset(cd8_2, subset = CDR3_group !="ORF3a-28_NA")
# 
# cd8_2 <- subset(cd8_2, subset = CDR3_group !="NP16_09")
# cd8_2 <- subset(cd8_2, subset = CDR3_group !="NP16_10")
# cd8_2 <- subset(cd8_2, subset = CDR3_group !="NP16_13")
# cd8_2 <- subset(cd8_2, subset = CDR3_group !="NP16_14")
# cd8_2 <- subset(cd8_2, subset = CDR3_group !="NP16_15")
# cd8_2 <- subset(cd8_2, subset = CDR3_group !="NP16_16")
# cd8_2 <- subset(cd8_2, subset = CDR3_group !="NP16_17")
# cd8_2 <- subset(cd8_2, subset = CDR3_group !="NP16_19")
# 
# cd8_2 <- subset(cd8_2, subset = CDR3_group !="ORF3a-28_03")
# cd8_2 <- subset(cd8_2, subset = CDR3_group !="ORF3a-28_04")
# cd8_2 <- subset(cd8_2, subset = CDR3_group !="ORF3a-28_06")
# 
# Idents(cd8_2) <- "CDR3_group"
# 
# VlnPlot(object = cd8_2, features=c("PTPRC", "CCR7", "SELL", "TCF7", "IL7R", "S100A4", 
#                                   "CD44","CD27", "CD28", "CD69", "CD38","HLA-DRB1",
#                                   "IFNG", "TNF", "CCL4", "CCL5", "CCR4", "CXCR3", "CXCR4",                                            "SELPLG", "ITGA4", "ITGAL", "ITGB2", "ITGAE", 
#                                   "GZMA", "GZMB", "GZMH", "GZMK", "PRF1", "GNLY",
#                                    "NKG7",  "ISG15", "TUBA1B","HAVCR2", "TIGIT", "CTLA4"),
#         ncol=6, group.by = "CDR3_group")

```


# Subset NP16 from CD8 object

```{r, fig.height=26, fig.width=18}
# Idents(cd8) <- "epitope"
# np16 <- subset(cd8, idents="NP16")
# 
# # list <- as.data.frame(np16$orig.ident)
# # list <- tibble::rownames_to_column(list, "cell_name")
# # list2 <- merge(list, cell_groups, by.x="cell_name", by.y="cell_name", all.x=TRUE)
# # 
# # np16@meta.data$avidity <- list2$avidity
# # np16@meta.data$CDR3_group <- list2$group
# 
# Idents(np16) <- "avidity"
# np16_2 <- subset(np16, idents=c("NP16 low avidity", "NP16 high avidity"))
# 
# table(Idents(np16_2))
# 
# VlnPlot(object = np16_2, features=c("PTPRC", "CCR7", "SELL", "TCF7", "IL7R", "S100A4",
#                                    "CD44","CD27", "CD28", "CD69", "CD38","HLA-DRB1",
#                                    "IFNG", "CCL4", "CCL5", "CCR4", "CXCR3", "CXCR4",                                                   "SELPLG", "ITGA4", "ITGAL", "ITGB2", "ITGAE", "ICAM1",
#                                    "GZMA", "GZMB", "GZMH", "GZMK", "PRF1", "GNLY",
#                                    "NKG7", "FASLG", "FAS",  "ISG15", "MX1", "OAS1",
#                                    "TUBA1B","LAG3","PDCD1", "HAVCR2", "TIGIT", "CTLA4",
#                                    "KLRG1",   "KLRD1", "FGFBP2", "KLRC1", "KLRC3", "KLRC2"),
#         ncol=6, group.by = "avidity")

```

```{r, fig.height=18, fig.width=16}
# Idents(np16) <- "clin.type"
# 
# VlnPlot(object = np16, features=c("PTPRC", "CCR7", "SELL", "TCF7", "IL7R", "S100A4", 
#                                   "CD44","CD27", "CD28", "CD69", "CD38","HLA-DRB1",
#                                   "IFNG", "TNF", "CCL4", "CCL5", "CCR4", "CXCR3", "CXCR4",                                            "SELPLG", "ITGA4", "ITGAL", "ITGB2", "ITGAE", 
#                                   "GZMA", "GZMB", "GZMH", "GZMK", "PRF1", "GNLY",
#                                    "NKG7",  "ISG15", "TUBA1B","HAVCR2", "TIGIT", "CTLA4"),
#         ncol=6, group.by = "clin.type")
```
## Harmony integration of CD8+ NP16

```{r, fig.height=8, fig.width=12}
# Select the top 1000 most variable features
# np16 <- FindVariableFeatures(object = np16, mean.function = ExpMean, dispersion.function = LogVMR, x.low.cutoff = 0.0125, x.high.cutoff = 3, y.cutoff = 0.5, nfeatures = 2000)
# 
# # Scaling RNA data
# np16 <- ScaleData(np16)
# 
# # Run PCA
# np16 <- RunPCA(np16, npcs = 30, verbose = FALSE)
# 
# # Select top PCs for clustering
# np16 <- FindNeighbors(np16, reduction = "pca", dims = 1:20, verbose=FALSE)
# np16 <- FindClusters(np16, resolution = 0.5, verbose = FALSE)
# np16 <- RunUMAP(np16, dims = 1:20, verbose=FALSE)
# 
# p1 <- DimPlot(np16, reduction = "umap", group.by = "orig.ident")
# p2 <- DimPlot(np16, reduction = "umap", group.by = "clin.type")
# p3 <- DimPlot(np16, reduction = "umap", group.by = "seurat_clusters")
# p4 <- DimPlot(np16, reduction = "pca", group.by = "orig.ident")
# 
# grid.arrange(p1, p2, p3, p4, nrow=2)

```
```{r}
# np16 <- np16 %>%
#     RunHarmony("orig.ident", plot_convergence = TRUE)
# 
# # access Harmony embeddings - show top 5 rows and top 5 columns
# harmony_embeddings <- Embeddings(np16, 'harmony')
```

```{r, fig.height=5, fig.width=12}
# p1 <- DimPlot(object = np16, reduction = "harmony", group.by = "orig.ident")
# p2 <- VlnPlot(object = np16, features = "harmony_1", group.by = "orig.ident", pt.size = .1)
# p1 + p2
```
```{r, fig.height=5, fig.width=12}
# p3 <- DimPlot(object = np16, reduction = "pca", group.by="orig.ident")
# p3 + p1
```


```{r, fig.height=5, fig.width=20}
# np16 <- np16 %>%
#     RunUMAP(reduction = "harmony", dims = 1:30) %>%
#     FindNeighbors(reduction = "harmony", dims = 1:30) %>%
#     FindClusters(resolution = 0.5) %>%
#     identity()
# umap_patient <- DimPlot(np16, reduction = "umap", group.by = "orig.ident")
# umap_clin <- DimPlot(np16, reduction = "umap", group.by = "clin.type")
# umap_cluster <- DimPlot(np16, reduction = "umap", group.by = "seurat_clusters")
# umap_patient + umap_clin + umap_cluster
```

# Subset ORF3a-28 from CD8 object

```{r, fig.height=32, fig.width=18}
# Idents(cd8) <- "epitope"
# orf3a <- subset(cd8, idents="ORF3a-28")
# # 
# # list <- as.data.frame(orf3a$orig.ident)
# # list <- tibble::rownames_to_column(list, "cell_name")
# # list2 <- merge(list, cell_groups, by.x="cell_name", by.y="cell_name", all.x=TRUE)
# # 
# # orf3a@meta.data$avidity <- list2$avidity
# # orf3a@meta.data$CDR3_group <- list2$group
# # 
# Idents(orf3a) <- "avidity"
# orf3a_2 <- subset(orf3a, idents=c("ORF3a-28 low avidity", "ORF3a-28 high avidity"))
# 
# table(Idents(orf3a_2))
# 
# VlnPlot(object = orf3a_2, features=c("PTPRC", "CCR7", "SELL", "TCF7", "IL7R", "S100A4",
#                                    "CD44","CD27", "CD28", "CD69", "CD38","HLA-DRB1",
#                                    "IFNG", "CCL4", "CCL5", "CCR4", "CXCR3", "CXCR4",                                                   "SELPLG", "ITGA4", "ITGAL", "ITGB2", "ITGAE", "ICAM1",
#                                    "GZMA", "GZMB", "GZMH", "GZMK", "PRF1", "GNLY",
#                                    "NKG7", "FASLG", "FAS",  "ISG15", "MX1", "OAS1",
#                                    "TUBA1B","LAG3","PDCD1", "HAVCR2", "TIGIT", "CTLA4",
#                                    "KLRG1",   "KLRD1", "FGFBP2", "KLRC1", "KLRC3", "KLRC2"),
#         ncol=6, group.by = "avidity")

```

# TCR diversity/similarity
Previously clustered single cells and clones by unsupervised hierarchical clustering. Look at single cell expression for similar/dissimilar sequences (linked to functional avidity)
```{r}
cd8 <- readRDS("cd8_harmony.rds")

clust <- read.csv("/t1-data/user/lfelce/TCR_analysis/diversity/subdendrograms_cd8_np16_orf3a.csv")

clust$epitope <- ifelse(grepl("NP16|B7_SPR", clust$correct_labels), "NP16", 
                        ifelse(grepl("ORF3a", clust$correct_labels), "ORF3a-28", ""))
  
clust <- mutate(clust, cluster=paste(epitope, sub_dendrogram, sep=" "))

clust$hclust <- ifelse(grepl("NP16 2|NP16 3|NP16 4|NP16 7", clust$cluster),"NP16 High",
                ifelse(grepl("NP16 1|NP16 5|NP16 6|NP16 8", clust$cluster),"NP16 Low",
                ifelse(grepl("ORF3a-28 1|ORF3a-28 2|ORF3a-28 6", clust$cluster), "ORF3a-28 High",
                ifelse(grepl("ORF3a-28 3|ORF3a-28 4|ORF3a-28 8", clust$cluster), "ORF3a-28 Low",
                ifelse(grepl("ORF3a-28 5|ORF3a-28 7", clust$cluster), "ORF3a-28 NA", "")))))

clust <- clust[!duplicated(clust$correct_labels),]

list <- as.data.frame(cd8$orig.ident)
list <- tibble::rownames_to_column(list, "cell_name")

clusters <- merge(list, clust, by.x="cell_name", by.y="correct_labels", all.x=TRUE)

clusters <- clusters[c(1:227, 290:740, 228:289),]

cd8 <- AddMetaData(object = cd8, metadata = clusters$hclust, col.name = "hclust")

p1 <- DimPlot(cd8, reduction = "umap", group.by = "orig.ident")
p2 <- DimPlot(cd8, reduction = "umap", group.by = "epitope")
p3 <- DimPlot(cd8, reduction = "umap", group.by = "seurat_clusters")
p4 <- DimPlot(cd8, reduction = "umap", group.by = "timepoint")
p5 <- DimPlot(cd8, reduction = "umap", group.by = "clin.type")
p6 <- DimPlot(cd8, reduction = "umap", group.by = "hclust")


```

```{r, fig.width=16, fig.height=16}
grid.arrange(p1, p2, p3, p4, p5, p6,nrow=3)
```


## Violin plots
```{r, fig.height=26, fig.width=18}
Idents(cd8) <- "hclust"

cl <- subset(cd8, idents=c("NP16 High", "NP16 Low", "ORF3a-28 High", "ORF3a-28 Low"))


Idents(cl) <- "hclust"
table(Idents(cl))

VlnPlot(object = cl, features=c("PTPRC", "CCR7", "SELL", "TCF7", "IL7R", "S100A4",
                                         "CD44","CD27", "CD28", "CD69", "CD38","HLA-DRB1",
                                         "IFNG", "CCL4", "CCL5", "CCR4", "CXCR3", "CXCR4",                                                   "SELPLG", "ITGA4", "ITGAL", "ITGB2", "ITGAE", "ICAM1",
                                         "GZMA", "GZMB", "GZMH", "GZMK", "PRF1", "GNLY",
                                         "NKG7", "FASLG", "FAS",  "ISG15", "MX1", "OAS1",
                                         "TUBA1B","LAG3","PDCD1", "HAVCR2", "TIGIT", "CTLA4",
                                         "KLRD1", "FGFBP2", "KLRC1", "KLRC3", "KLRB1", "KLRC2"),
        ncol=6, group.by = "hclust")
```

```{r, fig.height=24, fig.width=16}
Idents(cl) <- "hclust"

np16_cl <- subset(cl, idents=c("NP16 High", "NP16 Low"))

Idents(np16_cl) <- "hclust"
table(Idents(np16_cl))


VlnPlot(object = np16_cl, features=c("PTPRC", "CCR7", "SELL", "TCF7", "IL7R", "S100A4",
                                   "CD44","CD27", "CD28", "CD69", "CD38","HLA-DRB1",
                                   "IFNG", "CCL4", "CCL5", "CCR4", "CXCR3", "CXCR4",                                                                 "SELPLG", "ITGA4", "ITGAL", "ITGB2", "ITGAE", "ICAM1",
                                   "GZMA", "GZMB", "GZMH", "GZMK", "PRF1", "GNLY",
                                   "NKG7", "FASLG", "FAS",  "ISG15", "MX1", "OAS1",
                                   "TUBA1B","LAG3","PDCD1", "HAVCR2", "TIGIT", "CTLA4",
                                   "KLRG1",   "KLRD1", "FGFBP2", "KLRC1", "KLRC3", "KLRC2"),
        ncol=6, group.by = "hclust")
```
```{r, fig.height=26, fig.width=16}
Idents(cl) <- "hclust"

orf3a_cl <- subset(cl, idents=c("ORF3a-28 High", "ORF3a-28 Low"))

Idents(orf3a_cl) <- "hclust"
table(Idents(orf3a_cl))


VlnPlot(object = orf3a_cl, features=c("PTPRC", "CCR7", "SELL", "TCF7", "IL7R", "S100A4",
                                   "CD44","CD27", "CD28", "CD69", "CD38","HLA-DRB1",
                                   "IFNG", "CCL4", "CCL5", "CCR4", "CXCR3", "CXCR4",                                                                 "SELPLG", "ITGA4", "ITGAL", "ITGB2", "ITGAE", "ICAM1",
                                   "GZMA", "GZMB", "GZMH", "GZMK", "PRF1", "GNLY",
                                   "NKG7", "FASLG", "FAS",  "ISG15", "MX1", "OAS1",
                                   "TUBA1B","LAG3","PDCD1", "HAVCR2", "TIGIT", "CTLA4",
                                   "KLRG1",   "KLRD1", "FGFBP2", "KLRC1", "KLRC3", "KLRC2"),
        ncol=6, group.by = "hclust")
```
# AddModuleScore
To enable some statistical testing - takes average expression of gene set and compares to control genes. A higher score means that gene set is more highly expressed than random control genes.

## NP16 vs ORF3a-28
```{r}
features_memory <- list(c("PTPRC", "CCR7", "SELL", "TCF7", "IL7R", "S100A4"))
features_activation <- list(c("CD44","CD27", "CD28", "CD69", "CD38","HLA-DRB1", "TUBA1B"))
features_cytotoxic_ifn <- list(c("GZMA", "GZMB", "GZMH", "GZMK", "PRF1", "GNLY","NKG7", "FASLG", "FAS",  "ISG15", "MX1", "OAS1"))
features_cytokines <- list(c("IFNG", "CCL4", "CCL5", "CCR4", "CXCR3", "CXCR4"))
features_integrins <- list(c("SELPLG", "ITGA4", "ITGAL", "ITGB2", "ITGAE", "ICAM1"))
features_exhaustion <- list(c("LAG3","PDCD1", "HAVCR2", "TIGIT", "CTLA4"))
features_nk <- list(c("KLRD1", "FGFBP2", "KLRC1", "KLRC3", "KLRB1", "KLRC2"))

cd8 <- AddModuleScore(object = cd8, 
                      features = features_memory, ctrl = 5,
                      name = "Features.Memory")
cd8 <- AddModuleScore(object = cd8, 
                      features = features_activation, ctrl = 5,
                      name = "Features.Activation")
cd8 <- AddModuleScore(object = cd8, 
                      features = features_cytotoxic_ifn, ctrl = 5,
                      name = "Features.Cytotoxicity")
cd8 <- AddModuleScore(object = cd8, 
                      features = features_cytokines, ctrl = 5,
                      name = "Features.Cytokines")
cd8 <- AddModuleScore(object = cd8, 
                      features = features_integrins, ctrl = 5,
                      name = "Features.Integrins")
cd8 <- AddModuleScore(object = cd8, 
                      features = features_exhaustion, ctrl = 5,
                      name = "Features.Exhaustion")
cd8 <- AddModuleScore(object = cd8, 
                      features = features_nk, ctrl = 5,
                      name = "Features.NK")

p1 <- ggplot(cd8@meta.data, aes(x=epitope, y=Features.Memory1, colour=epitope)) +
  geom_boxplot(width=0.5, outlier.shape=NA ) + geom_jitter(width=0.25) +
  labs(title="Memory score",x="Epitope", y = "Memory score") + theme_light()

p2 <- ggplot(cd8@meta.data, aes(x=epitope, y=Features.Activation1, colour=epitope)) +
  geom_boxplot(width=0.5, outlier.shape=NA ) + geom_jitter(width=0.25) +
  labs(title="Activation score",x="Epitope", y = "Activation score") + theme_light()

p3 <- ggplot(cd8@meta.data, aes(x=epitope, y=Features.Cytotoxicity1, colour=epitope)) +
  geom_boxplot(width=0.5, outlier.shape=NA ) + geom_jitter(width=0.25) +
  labs(title="Cytotoxicity score",x="Epitope", y = "Cytotoxicity score") + theme_light()

p4 <- ggplot(cd8@meta.data, aes(x=epitope, y=Features.Cytokines1, colour=epitope)) +
  geom_boxplot(width=0.5, outlier.shape=NA ) + geom_jitter(width=0.25) +
  labs(title="Cytokines score",x="Epitope", y = "Cytokines score") + theme_light()

p5 <- ggplot(cd8@meta.data, aes(x=epitope, y=Features.Integrins1, colour=epitope)) +
  geom_boxplot(width=0.5, outlier.shape=NA ) + geom_jitter(width=0.25) +
  labs(title="Integrins score",x="Epitope", y = "Integrins score") + theme_light()

p6 <- ggplot(cd8@meta.data, aes(x=epitope, y=Features.Exhaustion1, colour=epitope)) +
  geom_boxplot(width=0.5, outlier.shape=NA ) + geom_jitter(width=0.25) +
  labs(title="Exhaustion score",x="Epitope", y = "Exhaustion score") + theme_light()

p7 <- ggplot(cd8@meta.data, aes(x=epitope, y=Features.NK1, colour=epitope)) +
  geom_boxplot(width=0.5, outlier.shape=NA ) + geom_jitter(width=0.25) +
  labs(title="NK cell genes score",x="Epitope", y = "NK cell genes score") + theme_light()

```

```{r, fig.height=10, fig.width=18}
grid.arrange(p1, p2, p3, p4, p5, p6, p7, nrow=2)
```
## NP16 High v NP16 Low Functional Avidity

```{r}
Idents(cd8) <- "hclust"

np16_cl <- subset(cd8, idents=c("NP16 High", "NP16 Low"))

Idents(np16_cl) <- "hclust"
table(Idents(np16_cl))

n1 <- ggplot(np16_cl@meta.data, aes(x=hclust, y=Features.Memory1, colour=hclust)) +
  geom_boxplot(width=0.5, outlier.shape=NA ) + geom_jitter(width=0.25) +
  labs(title="Memory score",x="Functional Avidity", y = "Memory score") + theme_light()

n2 <- ggplot(np16_cl@meta.data, aes(x=hclust, y=Features.Activation1, colour=hclust)) +
  geom_boxplot(width=0.5, outlier.shape=NA ) + geom_jitter(width=0.25) +
  labs(title="Activation score",x="Functional Avidity", y = "Activation score") + theme_light()

n3 <- ggplot(np16_cl@meta.data, aes(x=hclust, y=Features.Cytotoxicity1, colour=hclust)) +
  geom_boxplot(width=0.5, outlier.shape=NA ) + geom_jitter(width=0.25) +
  labs(title="Cytotoxicity score",x="Functional Avidity", y = "Cytotoxicity score") + theme_light()

n4 <- ggplot(np16_cl@meta.data, aes(x=hclust, y=Features.Cytokines1, colour=hclust)) +
  geom_boxplot(width=0.5, outlier.shape=NA ) + geom_jitter(width=0.25) +
  labs(title="Cytokines score",x="Functional Avidity", y = "Cytokines score") + theme_light()

n5 <- ggplot(np16_cl@meta.data, aes(x=hclust, y=Features.Integrins1, colour=hclust)) +
  geom_boxplot(width=0.5, outlier.shape=NA ) + geom_jitter(width=0.25) +
  labs(title="Integrins score",x="Functional Avidity", y = "Integrins score") + theme_light()

n6 <- ggplot(np16_cl@meta.data, aes(x=hclust, y=Features.Exhaustion1, colour=hclust)) +
  geom_boxplot(width=0.5, outlier.shape=NA ) + geom_jitter(width=0.25) +
  labs(title="Exhaustion score",x="Functional Avidity", y = "Exhaustion score") + theme_light()

n7 <- ggplot(np16_cl@meta.data, aes(x=hclust, y=Features.NK1, colour=hclust)) +
  geom_boxplot(width=0.5, outlier.shape=NA ) + geom_jitter(width=0.25) +
  labs(title="NK cell genes score",x="Functional Avidity", y = "NK cell genes score") + theme_light()

```
```{r, fig.height=10, fig.width=18}
grid.arrange(n1, n2, n3, n4, n5, n6, n7, nrow=2)
```
## NP16 High v NP16 Low Functional Avidity

```{r}
Idents(cd8) <- "hclust"

orf3a_cl <- subset(cd8, idents=c("ORF3a-28 High", "ORF3a-28 Low"))

Idents(orf3a_cl) <- "hclust"
table(Idents(orf3a_cl))

n1 <- ggplot(orf3a_cl@meta.data, aes(x=hclust, y=Features.Memory1, colour=hclust)) +
  geom_boxplot(width=0.5, outlier.shape=NA ) + geom_jitter(width=0.25) +
  labs(title="Memory score",x="Functional Avidity", y = "Memory score") + theme_light()

n2 <- ggplot(orf3a_cl@meta.data, aes(x=hclust, y=Features.Activation1, colour=hclust)) +
  geom_boxplot(width=0.5, outlier.shape=NA ) + geom_jitter(width=0.25) +
  labs(title="Activation score",x="Functional Avidity", y = "Activation score") + theme_light()

n3 <- ggplot(orf3a_cl@meta.data, aes(x=hclust, y=Features.Cytotoxicity1, colour=hclust)) +
  geom_boxplot(width=0.5, outlier.shape=NA ) + geom_jitter(width=0.25) +
  labs(title="Cytotoxicity score",x="Functional Avidity", y = "Cytotoxicity score") + theme_light()

n4 <- ggplot(orf3a_cl@meta.data, aes(x=hclust, y=Features.Cytokines1, colour=hclust)) +
  geom_boxplot(width=0.5, outlier.shape=NA ) + geom_jitter(width=0.25) +
  labs(title="Cytokines score",x="Functional Avidity", y = "Cytokines score") + theme_light()

n5 <- ggplot(orf3a_cl@meta.data, aes(x=hclust, y=Features.Integrins1, colour=hclust)) +
  geom_boxplot(width=0.5, outlier.shape=NA ) + geom_jitter(width=0.25) +
  labs(title="Integrins score",x="Functional Avidity", y = "Integrins score") + theme_light()

n6 <- ggplot(orf3a_cl@meta.data, aes(x=hclust, y=Features.Exhaustion1, colour=hclust)) +
  geom_boxplot(width=0.5, outlier.shape=NA ) + geom_jitter(width=0.25) +
  labs(title="Exhaustion score",x="Functional Avidity", y = "Exhaustion score") + theme_light()

n7 <- ggplot(orf3a_cl@meta.data, aes(x=hclust, y=Features.NK1, colour=hclust)) +
  geom_boxplot(width=0.5, outlier.shape=NA ) + geom_jitter(width=0.25) +
  labs(title="NK cell genes score",x="Functional Avidity", y = "NK cell genes score") + theme_light()

```
```{r, fig.height=10, fig.width=18}
grid.arrange(n1, n2, n3, n4, n5, n6, n7, nrow=2)
```
## Clustering for just T cells with diversity indexes
```{r}
# # Select the top 2000 most variable features
# cd8_diversity <- FindVariableFeatures(object = cd8_diversity, mean.function = ExpMean, dispersion.function = LogVMR, x.low.cutoff = 0.0125, x.high.cutoff = 3, y.cutoff = 0.5, nfeatures = 2000)
# 
# # Scaling RNA data
# cd8_diversity <- ScaleData(cd8_diversity)
# 
# # Run PCA
# cd8_diversity <- RunPCA(cd8_diversity, npcs = 30, verbose = FALSE)
# 
# # Select top PCs for clustering
# cd8_diversity <- FindNeighbors(cd8_diversity, reduction = "pca", dims = 1:30, verbose=FALSE)
# cd8_diversity <- FindClusters(cd8_diversity, resolution = 0.5, verbose = FALSE)
# cd8_diversity <- RunUMAP(cd8_diversity, dims = 1:30, verbose=FALSE)
# 
# p1 <- DimPlot(cd8_diversity, reduction = "umap", group.by = "orig.ident")
# p2 <- DimPlot(cd8_diversity, reduction = "umap", group.by = "epitope")
# p3 <- DimPlot(cd8_diversity, reduction = "umap", group.by = "seurat_clusters")
# p4 <- DimPlot(cd8_diversity, reduction = "umap", group.by = "timepoint")
# p5 <- DimPlot(cd8_diversity, reduction = "umap", group.by = "clin.type")
# p6 <- DimPlot(cd8_diversity, reduction = "umap", group.by = "diversity")

```

```{r, fig.width=16, fig.height=16}
# grid.arrange(p1, p2, p3, p4, p5, p6, nrow=3)
```



# Save file

```{r save}
# save <- object at this point so don't have to redo all of the above again
# saveRDS(cd8, file = "cd8_seurat.rds")
# saveRDS(cd8.integrated, file="cd8_integrated_seurat.rds")

# saveRDS(cd8, file="cd8_harmony.rds")
# saveRDS(acute, file="acute_harmony.rds")
# saveRDS(conv, file="convalescent_harmony.rds")

```
