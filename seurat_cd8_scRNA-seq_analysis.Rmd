...
# select CD8 cells only - should have 911
...
cd8_data <- tcell_data2[,c(1:192, 481:1055, 1344:1487)]
...
```{r top 2 genes, fig.height=3, fig.width=7}
# Top 2 genes from each cluster
VlnPlot(object = cd8, features =c("CCR7", "GZMB"), ncol=2)
```
...
clin.type <- rep(c("mild", "severe", "severe", "severe", "mild"), 
                 times = c("133", "15", "74", "66", "75", "86"))
...
FeaturePlot(object = cd8, features = c("IL21", "POU2AF1", "PDCD1", "CXCL13", "CD200", "BTLA", "IL4", "IFNG"), cols = c("grey", "blue"), ncol = 3, reduction = "umap")
```
### Th1
```{r, fig.height=8, fig.width=16}
FeaturePlot(object = cd8, features = c("TBX21", "IFNG", "LTA", "TNF", "IL2"), cols = c("grey", "blue"), ncol = 3, reduction = "umap")
```

### Th2
```{r, fig.height=4, fig.width=14}
FeaturePlot(object = cd8, features = c("IL4", "IL5", "IL6", "IL9", "IL13"), cols = c("grey", "blue"), ncol = 3, reduction = "umap")
```
### Treg
```{r, fig.height=4, fig.width=16}
FeaturePlot(object = cd8, features = c("IL10", "IL35", "TGFB1", "LGALS1"), cols = c("grey", "blue"), ncol = 3, reduction = "umap")
```
### Memory
```{r, fig.height=12, fig.width=16}
FeaturePlot(object = cd8, features = c("PTPRC", "CCR7", "SELL", "TCF7", "STAT5", "CD44", "IL7R", "S100A4","CD154", "CD38", "CD137"), cols = c("grey", "blue"), ncol = 3, reduction = "umap")
```
### Activation and differentiation
```{r, fig.height=8, fig.width=16}
FeaturePlot(object = cd8, features = c("CD27", "CD28", "CD69", "ITGAE", "CD25"), cols = c("grey", "blue"), ncol = 3, reduction = "umap")
```
### Recruitment and migration
```{r, fig.height=8, fig.width=16}
FeaturePlot(object = cd8, features = c("CCL3", "CCL4", "CCL5", "CXCR3", "CXCR4", "XCR1"), cols = c("grey", "blue"), ncol = 3, reduction = "umap")
```

### Integrins
```{r, fig.height=4, fig.width=14}
FeaturePlot(object = cd8, features = c("ITGAE", "ITGB3", "CD107", "ITGB7"), cols = c("grey", "blue"), ncol = 3, reduction = "umap")
```
### Type I interferon
```{r, fig.height=8, fig.width=16}
FeaturePlot(object = cd8, features = c("OAS1", "MX1", "IFIT3", "IFI44L", "ISG15", "MX2"), cols = c("grey", "blue"), ncol = 3, reduction = "umap")
```
...
tcr <- read.csv("cd8_tcr.csv", row.names=1)
...
cd8@meta.data$"trav" <- as.factor(trav$TRAV)
cd8@meta.data$"traj" <- as.factor(traj$TRAJ)
cd8@meta.data$"trbv" <- as.factor(trbv$TRBV)
cd8@meta.data$"trbj" <- as.factor(trbj$TRBJ)
```

```{r trav umap, fig.height=5, fig.width=8}
UMAP_trav <- DimPlot(cd8, reduction = "umap", group.by = "trav")
UMAP_trav
```

```{r traj umap, fig.height=5, fig.width=8}
UMAP_traj <- DimPlot(cd8, reduction = "umap", group.by = "traj")
UMAP_traj
```

```{r trbv umap, fig.height=5, fig.width=8}
UMAP_trbv <- DimPlot(cd8, reduction = "umap", group.by = "trbv")
UMAP_trbv
```
```{r trbj umap, fig.height=5, fig.width=8}
UMAP_trbj <- DimPlot(cd8, reduction = "umap", group.by = "trbj")
UMAP_trbj
```
