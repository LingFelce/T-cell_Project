...
div <- read.csv("/t1-data/user/lfelce/TCR_analysis/diversity/cd8_np16_orf3a-28_combined_shannon.csv")

np16 <- div[!grepl("ORF3a-28", div$epitope),]
np16$div_group <- cut(np16$diversity, c(-Inf, 1, Inf),
                     c("Low", "High"))

orf3a <- div[!grepl("NP16", div$epitope),]
orf3a$div_group <- cut(orf3a$diversity, c(-Inf, 1.5, Inf),
                     c("Low", "High"))

div2 <- rbind(np16, orf3a)

div2 <- mutate(div2, div_epi_group=paste(epitope, div_group, sep=" "))

list <- as.data.frame(cd8$orig.ident)
list <- tibble::rownames_to_column(list, "cell_name")

diversity <- merge(list, div2, by="cell_name", all.x=TRUE)

diversity <- diversity[c(1:227, 290:740, 228:289),]

cd8 <- AddMetaData(object = cd8, metadata = diversity$div_epi_group, col.name = "diversity")
