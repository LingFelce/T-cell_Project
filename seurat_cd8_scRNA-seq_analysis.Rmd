...
np16 <- read.csv("/t1-data/user/lfelce/TCR_analysis/diversity/cd8_np16_hclust_single_cells.csv")
orf3a <- read.csv("/t1-data/user/lfelce/TCR_analysis/diversity/cd8_orf3a-28_hclust_single_cells.csv")

clust <- rbind(np16, orf3a)
clust$epitope <- ifelse(grepl("NP16|B7_SPR", clust$correct_labels), "NP16", 
                        ifelse(grepl("ORF3a", clust$correct_labels), "ORF3a-28", ""))
  
clust <- mutate(clust, cluster=paste(epitope, group, sep=" "))

clust$hclust <- ifelse(grepl("NP16 A|NP16 E|NP16 H", clust$cluster),"NP16 High",
                       ifelse(grepl("NP16 B|NP16 C|NP16 D|NP16 F|NP16 G", clust$cluster),"NP16 Low",
                              ifelse(grepl("ORF3a-28 A|ORF3a-28 E|ORF3a-28 F", clust$cluster), "ORF3a-28 High",
                                     ifelse(grepl("ORF3a-28 B|ORF3a-28 C|ORF3a-28 D|ORF3a-28 G|ORF3a-28 H", 
                                                  clust$cluster), "ORF3a-28 Low", ""))))
...
VlnPlot(object = np16_cl, features=c("CCR7", "SELL", "IL7R", "S100A4",
                                     "CD27", "CD28", "HLA-DRB1","TUBA1B",
                                     "CXCR3", "SELPLG", "ITGA4", "ITGB2",
                                     "GZMA", "GZMB", "GZMH", "GZMK"),
        ncol=4, group.by = "hclust")
```
```{r, fig.height=12, fig.width=10}
Idents(cl) <- "hclust"
...
VlnPlot(object = orf3a_cl, features=c("PTPRC", "CCR7", "SELL", "IL7R", 
                                      "S100A4", "CD27", "TUBA1B", "PDCD1",
                                      "IFNG", "CCL4", "ITGA4", "ITGAL",
                                      "GZMA", "GZMB", "GZMH", "GZMK"),
        ncol=4, group.by = "hclust")
```
