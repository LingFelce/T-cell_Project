...
# # 
# a1 <- ggplot(div_a, aes(x=patient, y=diversity_index, color=TCR)) +
#   geom_jitter(width=0.25) +
#   labs(title="Diversity of CD8 NP16 CDR3 alpha",x="Patient", y = "Shannon diversity index")
# 
# a2 <- ggplot(div_a, aes(x=timepoint, y=diversity_index, color=TCR)) +
#   geom_jitter(width=0.25)+
#   labs(title="Diversity of CD8 NP16 CDR3 alpha",x="Timepoint", y = "Shannon diversity index")
# 
# a3 <- ggplot(div_a, aes(x=severity, y=diversity_index, color=TCR)) +
#   geom_jitter(width=0.25)+
#   labs(title="Diversity of CD8 NP16 CDR3 alpha",x="Severity", y = "Shannon diversity index")
# 
# b1 <- ggplot(div_b, aes(x=patient, y=diversity_index, color=TCR)) +
#   geom_jitter(width=0.25)+
#   labs(title="Diversity of CD8 NP16 CDR3 beta",x="Patient", y = "Shannon diversity index")
# 
# b2 <- ggplot(div_b, aes(x=timepoint, y=diversity_index,color=TCR)) +
#   geom_jitter(width=0.25)+
#   labs(title="Diversity of CD8 NP16 CDR3 beta",x="Timepoint", y = "Shannon diversity index")
# 
# b3 <- ggplot(div_b, aes(x=severity, y=diversity_index, color=TCR)) +
#   geom_jitter(width=0.25)+
...
# grid.arrange(a1, b1, a2, b2, a3, b3, nrow=3)

```
```{r, fig.width=12, fig.height=6}
# Welch 2 sample t-test: (y~x) where y is numeric and x is a binary factor
# t.test(div_a$diversity_index~div_a$severity)
# 
# t.test(div_b$diversity_index~div_b$severity)
# 
# q1 <- ggplot(div_a, aes(x=severity, y=diversity_index)) +
#   geom_boxplot()+
# labs(title="Diversity of CD8+ NP16 & ORF3a-28 CDR3 alpha",x="Severity", y = "Shannon diversity index")
# 
# q2 <- ggplot(div_b, aes(x=severity, y=diversity_index)) +
#   geom_boxplot()+
...
# NP16 and ORF3a-28 combined only from 1 beta cells
...
sc_np16_b$cell_name <- paste("/t1-data/user/lfelce/TCR_analysis/cd8_np16_new/",sc_np16_b$cell_name, ".clonotypes.TRB.txt", sep="")

np16_list <- as.list(sc_np16_b$cell_name)


# parse in files with tcR
mixcr_b_np16 <- parse.file.list(np16_list, "mixcr")
...
sc_orf3a_b$cell_name <- paste("/t1-data/user/lfelce/TCR_analysis/cd8_orf_new/",sc_orf3a_b$cell_name, ".clonotypes.TRB.txt", sep="")

orf3a_list <- as.list(sc_orf3a_b$cell_name)


# # parse in files with tcR
# mixcr_b_orf3a <- parse.file.list(orf3a_list, "mixcr")
# 
# # combine mixcr files
# mixcr_b <- c(mixcr_b_np16, mixcr_b_orf3a)
# 
...
# trb <- big_data %>% group_by(i) %>% filter(n() <= 2)
# 
# colnames(trb) <- c("Read.count", "clone_fraction","CDR3_beta", "TRBV", "TRBJ", "cell_number")
# trb <- merge(trb, mixcr_b_names, by="cell_number")
# 
# # tidy up
# trb <- mutate(trb, beta=paste(TRBV, TRBJ, sep="_"))
```
## Shannon diversity index with tcR
```{r}
# div <- as.data.frame(repDiversity(.data = mixcr_b, .method = "entropy", .quant = "umi.prop"))
...
combined$patient <- ifelse(grepl("005", combined$cell_name), "005",
                          ifelse(grepl("1131-TP-1",combined$cell_name), "1131-TP-1",
                          ifelse(grepl("1131-TP-2", combined$cell_name), "1131-TP-2",
                          ifelse(grepl("1153", combined$cell_name), "1153",
                          ifelse(grepl("1201", combined$cell_name), "1201",
                          ifelse(grepl("1105", combined$cell_name), "1105",
                          ifelse(grepl("1134-TP-2",combined$cell_name), "1134-TP-2",
                          ifelse(grepl("1525-TP-1", combined$cell_name), "1525-TP-1",
                          ifelse(grepl("1525-TP-2", combined$cell_name), "1525-TP-2",
                                 "")))))))))

combined$severity <- ifelse(grepl("005", combined$cell_name), "mild",
                          ifelse(grepl("1131-TP-1",combined$cell_name), "severe",
                          ifelse(grepl("1131-TP-2", combined$cell_name), "severe",
                          ifelse(grepl("1153", combined$cell_name), "severe",
                          ifelse(grepl("1201", combined$cell_name), "mild",
                          ifelse(grepl("1105", combined$cell_name), "severe",
                          ifelse(grepl("1134-TP-2",combined$cell_name), "severe",
                          ifelse(grepl("1525-TP-1", combined$cell_name), "severe",
                          ifelse(grepl("1525-TP-2", combined$cell_name), "severe",
                                 "")))))))))

combined$timepoint <- ifelse(grepl("TP-1", combined$patient), "acute",
                   ifelse(grepl("005|TP-2|1153|1201|1105", combined$cell_name), "convalescent", ""))

combined$epitope <- ifelse(grepl("NP16|B7_SPR", combined$cell_name), "NP16",
                 ifelse(grepl("ORF3a-28", combined$cell_name), "ORF3a-28", ""))
                           

datalist = list()
for (i in (1:length(combined$cell_name))) {
  dat <- stringsim(combined$CDR3_beta[i], combined$CDR3_beta, method="lv") 
  datalist[[i]] <- dat # add it to list
}
big_data = do.call(rbind, datalist)

colnames(big_data) <- combined$patient
rownames(big_data) <- combined$cell_name

annotation <- data.frame(Epitope = combined$epitope, Severity = combined$severity)

rownames(annotation) <- combined$cell_name

```

```{r,, fig.width=20, fig.height=15}

pheatmap(big_data, show_rownames=FALSE, annotation_row = annotation)
